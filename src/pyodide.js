...
! function(global, factory) {
  ...
  ! function(module, exports) {
      module.exports = function() {
          ...
          function StackFrame(obj) {
              if (obj)
                  for (var i = 0; i < props.length; i++) void 0 !== obj[props[i]] && this["set" + _capitalize(props[i])](obj[props[i]])
          }
  ...
  //         return new Uint8Array(await response.arrayBuffer())
  //     }, globalThis.document) loadScript = async url => await import( /* webpackIgnore: true */ url);
  // else if (globalThis.importScripts) loadScript = async url => {
  //     try {
  ...
  //         url.includes("://") ? nodeVmMod.runInThisContext(await (await nodeFetch(url)).text()) : await import( /* webpackIgnore: true */ nodeUrlMod.pathToFileURL(url).href)
  //     }
  // }
  ...
  async function loadPyodide(options = {}) {
      ...
      const default_config = {
          ...
          pyodide_py_tar_promise = loadBinaryFile(config.indexURL + "pyodide_py.tar"),
          Module = {
              noImageDecoding: !0,
              noAudioDecoding: !0,
              noWasmDecoding: !1,
      ...
      Module.API = API, setStandardStreams(Module, config.stdin, config.stdout, config.stderr),
